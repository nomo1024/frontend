...
const HumiditySensorPage: React.FC = () => {
  ...
  const fetchStatus = async () => {
    try {
      const res = await getSensorStatus(2);
      console.log('湿度 状态响应:', res);
      let running = false;
      if (typeof res === 'boolean') running = res;
      else if (typeof res === 'string') running = res === '运行中' || res === 'running';
      else if (res) {
        if (res.source_2) running = res.source_2 === '运行中';
        else if (res.running !== undefined) running = res.running === true;
        else if (res.data) running = parseStatus(res.data, 2);
      }
      setIsRunning(running);
    } catch (error) {
      console.error('湿度 获取状态失败', error);
    }
  };
  ...
  const fetchInitialData = async () => {
    try {
      const res = await readSensorData(2, 5);
      console.log('湿度初始数据:', JSON.stringify(res, null, 2));
      if (res && Array.isArray(res)) {
        const formatted = res.map((item: any) => ({
          time: item.collect_time || new Date(item.createTime || Date.now()).toLocaleTimeString(),
          value: parseFloat(item.col1 || item.value || item.humidityValue || 0),
        }));
  ...
  const fetchData = async () => {
    try {
      const res = await readSensorData(2, 1);
      console.log('湿度最新数据:', JSON.stringify(res, null, 2));
      if (res && Array.isArray(res)) {
        const newItem = {
          time: res[0].collect_time || new Date(res[0].createTime || Date.now()).toLocaleTimeString(),
          value: parseFloat(res[0].col1 || res[0].value || res[0].humidityValue || 0),
        };
        setChartData(prev => {
  ...
  return (
    <Card
      title={<div style={{ textAlign: 'center', width: '100%' }}>湿度监测</div>}
      extra={
        <Button
